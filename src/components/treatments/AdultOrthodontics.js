...
const AdultOrthodontics = () => {
  ...
  return (
<div className="blog-slider w-95 relative max-w-800 mx-auto bg-white shadow-lg p-8 rounded-md h-400 overflow-hidden">
  <AnimatePresence exitBeforeEnter={false}>
    <motion.div
      ...
      className="blog-slider__item flex items-center flex-col"
      style={{ position: 'relative' }}
    >
      <div className="blog-slider__img" style={{ width: '300px', height: '300px', overflow: 'hidden', borderRadius: '20px', transform: 'translateX(-80px)' }}>
        <img src={sliderItems[currentSlide].image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', borderRadius: '20px', transition: 'all 0.3s' }} />
      </div>
      <div className="blog-slider__content text-center" style={{ padding: '0 25px', textAlign: 'center' }}>
        <span className="blog-slider__code">{sliderItems[currentSlide].date}</span>
        <div className="blog-slider__title">{sliderItems[currentSlide].title}</div>
        <div className="blog-slider__text">{sliderItems[currentSlide].text}</div>
      </div>
    </motion.div>
  </AnimatePresence>
  <div className="blog-slider__pagination mt-4">
    {[...Array(sliderItems.length)].map((_, index) => (
      <div
        key={index}
        className="blog-slider__bullet"
        style={{
          width: '10px',
          height: '10px',
          marginRight: '5px',
          borderRadius: '50%',
          backgroundColor: currentSlide === index ? '#fff' : '#000',
          cursor: 'pointer',
        }}
